...
router
  ...
  .patch(async (req, res) => {
    ...
    if (Object.keys(newInfo).length === 0){
      return res.status(400).json({error: 'Error: PATCH must be called with at least one key to update.'})
    }
    try{
    ...
    if (!helpers.sessionComparator(thisUser, oldPost.userThatPosted)){
      return res.status(403).json({error: 'Error: User cannot update post by a different user.'})
    }
    const params = {
      title: oldPost.title,
      text: oldPost.text
    ...
    if (!changes) return res.status(400).json({error: 'Error: Post PATCH route called with no changes to the post.'});

    try {
      const updatedPost = await postData.updateRecipe(postId, params.title, params.text, thisUser);
...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      const updatedPost = await postData.addCommentToPost(postId, thisUser, comment);
      return res.json(updatedPost);
    } catch (e) {
      return res.status(500).json({error: e});
    }
...
router
  ...
  .delete(async (req, res) => {
    ...
    try {
      ...
      for (let comment of post.comments){
        if (comment._id === commentId){
          if (helpers.sessionComparator(thisUser, comment.userThatPostedComment)){
            commentFound = true;
            break;
          } else {
    ...
    try {
      const updatedPost = await postData.deleteComment(postId, commentId, thisUser);
      return res.json(updatedPost);
    } catch (e) {
      return res.status(500).json({error: e});
    }
